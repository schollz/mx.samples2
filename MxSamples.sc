	...
	*new {
	...
	init {
		...
		SynthDef("playx2",{
			...
			var snd,snd2;
			snd=PlayBuf.ar(2,buf1,rate,t_trig,startPos:startPos*BufFrames.ir(buf1));
			snd2=PlayBuf.ar(2,buf2,rate,t_trig,startPos:startPos*BufFrames.ir(buf2));
	...
	noteOff {
		...
		if (syn.at(note).notNil,{
			if (syn.at(note).isRunning,{
				syn.at(note).set(\fade_trig,1,\fade_time,decay);
			});
		});
	}
			...
			Buffer.read(server,PathName(folder+/+file1).fullPath,action:{ arg b1;
				...
				if (buf.at(file2).isNil,{
					Buffer.read(server,PathName(folder+/+file2).fullPath,action:{ arg b1;
						...
						buf.put(file2,b1);
						// play it!
						this.doPlay(note,amp,file1,file2,buf1mix,rate);
					});
				},{
					// play it!
					this.doPlay(note,amp,file1,file2,buf1mix,rate);
	...
	doPlay {
		...
		[note,amp,file1,file2,buf1mix,rate].postln;
		// check if sound is loaded and unload it
		this.noteOff(note);
		syn.put(note,Synth("playx2",[
			\out,0,
			\amp,amp,
			\buf1,buf.at(file1),
			\buf2,buf.at(file2),
		...
		NodeWatcher.register(syn.at(note));
	}
